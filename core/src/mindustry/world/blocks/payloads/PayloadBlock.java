...
public class PayloadBlock extends Block{
    ...
    public static boolean blends(Building build, int direction){
        ...
        return accept != null &&
            ...
            && ((accept.block.rotate && Point2.equals(Hex.nearby(accept.tileX(), accept.tileY(), accept.rotation, size).x, Hex.nearby(accept.tileX(), accept.tileY(), accept.rotation, size).y, build.tileX(), build.tileY()))
            || !accept.block.rotate
            || !accept.block.outputFacing)) ||

            //if the other block is smaller, check alignment
            (accept.block.size != size &&
            //check orientation; make sure it's aligned properly with this block.
